...
│                                                                     │
│  ┌──────────┐  ┌──────────────┐  ┌──────────────┐  ┌────────────┐ │
│  │  Landing  │  │  Auth/Reset  │  │  Shared View │  │  Explorer  │ │
│  └──────────┘  └──────────────┘  └──────────────┘  └────────────┘ │
│                                                                     │
│  ┌────────────────┐ ┌────────────────┐ ┌─────────────────┐         │
│  │ Issuer Portal   │ │ Holder Wallet  │ │ Verifier Portal │         │
│  │ • Schemas (v)   │ │ • DID Mgmt     │ │ • Verify VP     │         │
│  │ • Issue (sign)  │ │ • Biometrics   │ │ • OID4VP Request│         │
│  │ • Batch CSV     │ │ • Selective    │ │ • DID Resolver  │         │
│  │ • OID4VCI Offer │ │   Disclosure   │ │ • Trust Registry│         │
│  │ • Revoke        │ │ • Export VC    │ │ • AI Analysis   │         │
│  │ • Trust Registry│ │ • MFA Verify   │ │ • History       │         │
│  │ • Audit Trail   │ │ • Privacy/GDPR │ │ • Analytics     │         │
│  │ • Analytics     │ │ • Web3 Wallet  │ │                 │         │
│  └────────────────┘ └────────────────┘ └─────────────────┘         │
└──────────────────────────────┬──────────────────────────────────────┘
                               │ Supabase JS Client
                               ▼
...
│  │  (Email/Pass)    │  │  12 tables, 30+ policies │                   │
│  └─────────────────┘  └─────────────────────────┘                   │
│                                                                     │
│  ┌─────────────────────────────────────────────────────────────┐    │
│  │                   Edge Functions (Deno)                      │    │
...
│  │  │ • W3C VC + proof │ │ • Hash integrity │                  │    │
│  │  │ • Wallet signing │ │ • Status check   │                  │    │
│  │  │ • Hash chain     │ │ • AI analysis    │                  │    │
│  │  │ • Batch support  │ │ • Audit logging  │                  │    │
│  │  └──────────────────┘ └──────────────────┘                  │    │
│  │  ┌──────────────────┐ ┌──────────────────┐                  │    │
│  │  │   resolve-did    │ │     oid4vci      │                  │    │
│  │  │ • did:decentraid │ │ • Credential Offer│                 │    │
│  │  │ • did:ethr       │ │ • Token Endpoint │                  │    │
│  │  │ • Trust metadata │ │ • Cred Endpoint  │                  │    │
│  │  └──────────────────┘ └──────────────────┘                  │    │
│  │  ┌──────────────────┐                                       │    │
│  │  │     oid4vp       │                                       │    │
│  │  │ • Auth Request   │                                       │    │
...
│                                                                     │
│  ┌──────────────────┐  ┌──────────────────────┐                     │
│  │ Realtime Channels│  │ Lovable AI Gateway   │                     │
│  │ (credentials,    │  │ (Gemini 3 Flash)     │                     │
│  │  notifications)  │  │ Credential analysis  │                     │
│  └──────────────────┘  └──────────────────────┘                     │
└─────────────────────────────────────────────────────────────────────┘
```
...
```
1. User signs up → selects role (issuer/holder/verifier)
...
```
Issuer creates offer → QR code displayed
     ↓
...
```
openid-credential-offer://?credential_offer={"credential_issuer":"...","credentials":["certificate"],"grants":{"urn:ietf:params:oauth:grant-type:pre-authorized_code":{"pre-authorized_code":"..."}}}
```
...
```
Verifier creates request → QR code displayed
     ↓
...
```
openid4vp://?client_id=...&response_type=vp_token&response_uri=...&response_mode=direct_post&presentation_definition={...}
```
...
```
Issuer fills form + enables wallet signing
        ↓
...
```
Issuer creates offer (schema + data)
        ↓
...
```
Verifier creates request (credential_types, purpose)
        ↓
...
```
Holder selects credential → toggles selective mode
        ↓
...
```
Verifier pastes VP JSON → POST /verify-credential
        ↓
...
*Platform: DecentraID — Decentralized Identity for Education*
*Built with: Lovable + React + Lovable Cloud*
*Standards: W3C VC, W3C DID, OpenID4VCI, OpenID4VP, StatusList2021*
