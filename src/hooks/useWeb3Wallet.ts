...
export function useWeb3Wallet(userId: string | undefined, options: UseWeb3WalletOptions = {}) {
  ...
  const autoGenerateDid = useCallback(async (userId: string) => {
    if (didGeneratedRef.current || isAutoGeneratingDid) return;
    setIsAutoGeneratingDid(true);
    try {
      const { data, error } = await supabase.rpc("generate_did", { _user_id: userId });
      if (error) throw error;
      if (data) {
        didGeneratedRef.current = true;
        toast({ title: "DID Generated", description: `Your decentralized identifier has been created from your wallet.` });
      }
    } catch (err: any) {
      toast({ title: "DID Generation Failed", description: err.message, variant: "destructive" });
    } finally {
